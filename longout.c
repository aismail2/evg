...
initRecord(longoutRecord *record)
{
	...
	for (i = 1; strlen(tokens[i]); i++)
	{
		...
		if (strcmp(key, "address") == 0)
			outputs[outputCount].address	=	atoi(value);
		else if (strcmp(key, "counter") == 0)
			outputs[outputCount].counter	=	atoi(value);
		else
...
thread(void* arg)
{
	...
	if (strcmp(private->command, "setRfPrescaler") == 0)
		status	=	evg_setRfPrescaler(private->device, record->val);
	else if (strcmp(private->command, "setSequencerPrescaler") == 0)
		status	=	evg_setSequencerPrescaler(private->device, record->val);
	else if (strcmp(private->command, "setAcTriggerPrescaler") == 0)
		status	=	evg_setAcTriggerPrescaler(private->device, record->val);
		...
		status	=	evg_setEvent(private->device, private->address, record->val);
	else if (strcmp(private->command, "setTimestamp") == 0)
		status	=	evg_setTimestamp(private->device, private->address, record->val);
	else
...
epicsExportAddress(dset, longoutevg);
